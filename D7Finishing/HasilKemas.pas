...
procedure THasilKemasFrm.FormClose(Sender: TObject;
...
procedure THasilKemasFrm.FormResize(Sender: TObject);
begin
...
procedure THasilKemasFrm.wwDBGrid1Enter(Sender: TObject);
begin
  If QMaster.State<> dsBrowse then QMaster.Post;
end;

procedure THasilKemasFrm.FormCreate(Sender: TObject);
begin
...
procedure THasilKemasFrm.QMasterNewRecord(DataSet: TDataSet);
begin
...
procedure THasilKemasFrm.LookRekananEnter(Sender: TObject);
begin
 DMFrm.QRekanan.Open;
end;

procedure THasilKemasFrm.LookRekananClick(Sender: TObject);
begin
   DMFrm.QRekanan.Close;
   DMFrm.QRekanan.Open;
end;

procedure THasilKemasFrm.QMasterBeforePost(DataSet: TDataSet);
begin
...
procedure THasilKemasFrm.QDetailBeforeEdit(DataSet: TDataSet);
begin
...
procedure THasilKemasFrm.LookKonstruksiEnter(Sender: TObject);
begin
  DMFrm.QKonstruksi.Open;
end;

procedure THasilKemasFrm.LookCorakEnter(Sender: TObject);
begin
  DMFrm.QCorak.Open;
end;

procedure THasilKemasFrm.LookMutuEnter(Sender: TObject);
begin
  DMFrm.QMutu.Open;
end;

procedure THasilKemasFrm.LookKonstruksiCloseUp(Sender: TObject;
...
procedure THasilKemasFrm.LookCorakCloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
  If modified then
...
procedure THasilKemasFrm.Edit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=13 then
  begin
...
procedure THasilKemasFrm.wwDBNavigator1ButtonClick(Sender: TObject);
begin
  QDetail.Close;
  LookMaster.Execute;
  QDetail.Open;
...
procedure THasilKemasFrm.QDetailNewRecord(DataSet: TDataSet);
begin
...
procedure THasilKemasFrm.BitBtn2Click(Sender: TObject);
begin
...
procedure THasilKemasFrm.ColumnHeaderBand1BeforePrint(
...
procedure THasilKemasFrm.FNo_UrutBeforeQuery(
...
procedure THasilKemasFrm.LookOperatorEnter(Sender: TObject);
begin
  DMFrm.QOperator.Open;
end;

procedure THasilKemasFrm.LookOperatorCloseUp(Sender: TObject;
  LookupTable, FillTable: TDataSet; modified: Boolean);
begin
  if modified then
...
procedure THasilKemasFrm.QMasterBeforeQuery(Sender: TOracleDataSet);
begin
  QMaster.SetVariable('kd_transaksi',QTransaksiKD_TRANSAKSI.AsString);
end;

procedure THasilKemasFrm.wwDBGrid1UpdateFooter(Sender: TObject);
begin
  QTotal.Close;
