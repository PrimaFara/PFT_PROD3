...
type
  TKoreksiFrm = class(TForm)
    ...
    QMasterCEK: TStringField;
    Button3: TButton;
    QHandprint1: TOracleQuery;
    QHandprint2: TOracleQuery;
    QCekHandprint: TOracleDataSet;
    QCekHandprintCEK: TFloatField;
    QCorakHandprint: TOracleDataSet;
...
procedure TKoreksiFrm.FormClose(Sender: TObject;
...
procedure TKoreksiFrm.FormResize(Sender: TObject);
begin
...
procedure TKoreksiFrm.wwDBGrid1Enter(Sender: TObject);
begin
  If QMaster.State<> dsBrowse then QMaster.Post;
end;

procedure TKoreksiFrm.FormCreate(Sender: TObject);
begin
 QTransaksi.SetVariable('kd_transaksi','918');
 QTransaksi.Open;
 KoreksiFrm.Caption:=QTransaksiNAMA_TRANSAKSI.AsString;
 QMaster.Open;
 QDetail.Open;
end;

procedure TKoreksiFrm.LookRekananEnter(Sender: TObject);
begin
 //DMFrm.QRekanan.Open;
end;

procedure TKoreksiFrm.LookRekananClick(Sender: TObject);
begin
   //DMFrm.QRekanan.Close;
   //DMFrm.QRekanan.Open;
end;

procedure TKoreksiFrm.QMasterBeforePost(DataSet: TDataSet);
begin
...
procedure TKoreksiFrm.QDetailBeforeEdit(DataSet: TDataSet);
begin
...
procedure TKoreksiFrm.Edit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=13 then
  begin
...
procedure TKoreksiFrm.wwDBNavigator1ButtonClick(Sender: TObject);
begin
  QDetail.Close;
  LookMaster.Execute;
  QDetail.Open;
...
procedure TKoreksiFrm.QDetailNewRecord(DataSet: TDataSet);
begin
...
procedure TKoreksiFrm.BitBtn2Click(Sender: TObject);
begin
...
procedure TKoreksiFrm.ColumnHeaderBand1BeforePrint(
...
procedure TKoreksiFrm.FNo_UrutBeforeQuery(
...
procedure TKoreksiFrm.QMasterBeforeQuery(Sender: TOracleDataSet);
begin
  QMaster.SetVariable('kd_transaksi',QTransaksiKD_TRANSAKSI.AsString);
end;

procedure TKoreksiFrm.VTglAwalChange(Sender: TObject);
begin
  vTglAkhir.Date:=trunc(EndOfTheMonth(VTglAwal.Date));
end;

procedure TKoreksiFrm.vTglAkhirChange(Sender: TObject);
begin
...
procedure TKoreksiFrm.cbOtomatisClick(Sender: TObject);
begin
...
procedure TKoreksiFrm.dbcFieldEnter(Sender: TObject);
var
  i : Word;
begin
...
procedure TKoreksiFrm.vOperandClick(Sender: TObject);
begin
...
procedure TKoreksiFrm.cbTanggalClick(Sender: TObject);
begin
...
procedure TKoreksiFrm.BitBtn3Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TKoreksiFrm.BtnExportClick(Sender: TObject);
begin
          if QBrowse.Active then
          begin
...
procedure TKoreksiFrm.wwDBGrid1xxxxxTitleButtonClick(
...
procedure TKoreksiFrm.TabSheet1Show(Sender: TObject);
begin
         QMaster.Close;
         ...
         if wwDBComboBox3.ItemIndex <> 2 then
         begin
         ...
         if ((DMfrm.QUserTimeUSER_OPR.AsString = 'DENNY') or
            (DMfrm.QUserTimeUSER_OPR.AsString = 'ANGGIT') or
            (DMfrm.QUserTimeUSER_OPR.AsString = 'MULKI') or
            (DMfrm.QUserTimeUSER_OPR.AsString = 'DIDIN')) and
            (QMasterISPOST.AsString='1') and (QMasterJNS_KOREKSI.AsString='LAIN-LAIN KELUAR') then
         begin
            QCekHandprint.Close;
            QCekHandprint.SetVariable('no_nota',QMasterNO_NOTA.AsString);
            QCekHandprint.Open;
            Button3.Visible:=true;
            if QCekHandprintCEK.AsInteger > 0 then
              Button3.Caption:='Sdh Cek Handprint'
              else
              Button3.Caption:='Blm Cek Handprint';
         end
         else
...
procedure TKoreksiFrm.wwDBGrid1UpdateFooter(Sender: TObject);
begin
  QTotal.Close;
...
procedure TKoreksiFrm.FormShow(Sender: TObject);
begin
{azmi}
...
procedure TKoreksiFrm.QBrowseAfterScroll(DataSet: TDataSet);
begin
{azmi}
...
procedure TKoreksiFrm.wwDBGrid1xxxxxEnter(Sender: TObject);
begin
  If QMaster.State<> dsBrowse then QMaster.Post;
 
end;

procedure TKoreksiFrm.LookKonstruksiCloseUp(Sender: TObject;
...
procedure TKoreksiFrm.LookCorakCloseUp(Sender: TObject;
  LookupTable, FillTable: TDataSet; modified: Boolean);
begin
 If modified then
...
procedure TKoreksiFrm.LookKonstruksiEnter(Sender: TObject);
begin
QLookKp.Close;
...
procedure TKoreksiFrm.LookCorakEnter(Sender: TObject);
begin
  DMFrm.QCorak.Open;
end;

procedure TKoreksiFrm.LookKonstruksiClick(Sender: TObject);
begin
//   DMFrm.QLookKdProduksi.Open;
  QLookKp.Open;
end;

procedure TKoreksiFrm.LookCorakClick(Sender: TObject);
begin
   DMFrm.QCorak.Open;
end;

procedure TKoreksiFrm.QMasterNewRecord(DataSet: TDataSet);
begin
DMFrm.QUserTime.Open;
...
procedure TKoreksiFrm.LookMesinFinishingEnter(Sender: TObject);
begin
...
procedure TKoreksiFrm.LookMesinFinishingCloseUp(Sender: TObject;
  LookupTable, FillTable: TDataSet; modified: Boolean);
begin
...
procedure TKoreksiFrm.VTglAwal2Change(Sender: TObject);
begin
 VTglakhir2.Date:=(VTglAwal2.Date);
end;

procedure TKoreksiFrm.VTglakhir2Change(Sender: TObject);
begin
...
procedure TKoreksiFrm.dbcField2Enter(Sender: TObject);
var
  i : Word;
begin
...
procedure TKoreksiFrm.BitBtn5Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TKoreksiFrm.BtnExport2Click(Sender: TObject);
begin
if Qkon_opening.Active then
          begin
             DMFrm.SaveDialog1.DefaultExt:='XLK';
...
procedure TKoreksiFrm.BitBtn6Click(Sender: TObject);
begin
DMFrm.QUserTime.Close;
 DMFrm.QUserTime.Open;
...
procedure TKoreksiFrm.QRBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
QRLabel26.Caption:='(Per : '+vTglAwal2.Text+' s/d '+vTglAkhir2.Text+')';
end;

procedure TKoreksiFrm.wwDBLookupComboDlg1Enter(Sender: TObject);
begin
DMFrm.QCorak.Open;
end;

procedure TKoreksiFrm.wwDBLookupComboDlg1Click(Sender: TObject);
begin
DMFrm.QCorak.Open;
end;

procedure TKoreksiFrm.wwDBLookupComboDlg1CloseUp(Sender: TObject;
  LookupTable, FillTable: TDataSet; modified: Boolean);
begin
if modified then
...
procedure TKoreksiFrm.TabSheet3Show(Sender: TObject);
begin
QBrowse.Open;
Qkon_opening.Open;
end;

procedure TKoreksiFrm.wwDBComboBox1CloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
QMasterGRUP.AsString:=DMFrm.QLookGrupGRUP.AsString;
end;

procedure TKoreksiFrm.wwDBComboBox1Enter(Sender: TObject);
begin
DMFrm.QLookGrup.Open;
end;

procedure TKoreksiFrm.QDetailBeforeInsert(DataSet: TDataSet);
begin
if (QDetailQTY_POTONG.AsFloat > QLookKPSTOK.AsFloat) and 
   (wwDBComboBox3.Text <> 'KOREKSI MASUK') and 
   (wwDBComboBox3.Text <> 'RETUR MASUK') and 
   (wwDBComboBox3.Text <> 'LAIN-LAIN MASUK') and 
   (wwDBComboBox3.Text <> 'PENYESUAIAN MASUK') then
  begin
...
procedure TKoreksiFrm.Button1Click(Sender: TObject);
begin
...
procedure TKoreksiFrm.wwDBComboBox3CloseUp(Sender: TwwDBComboBox;
  Select: Boolean);
begin
if wwDBComboBox3.ItemIndex = 2 then
         begin
...
procedure TKoreksiFrm.Button2Click(Sender: TObject);
begin
QMaster.Edit;
QMaster.Post;
QAmbilNota.Close;
QAmbilNota.SetVariable('no_reg',QMasterNO_REG.AsString);
QAmbilNota.SetVariable('nota',wwDBEdit3.Text);
QAmbilNota.Execute;
QDetail.Close;
QDetail.Open;
end;

procedure TKoreksiFrm.Button3Click(Sender: TObject);
begin
  QCekHandprint.Close;
  QCekHandprint.SetVariable('no_nota', QMasterNO_NOTA.AsString);
  QCekHandprint.Open;
  ...
  begin
    QHandprint2.Close;
    QHandprint2.SetVariable('no_nota', QMasterNO_NOTA.AsString);
    QHandprint2.SetVariable('no_reg', QMasterNO_REG.AsString);
    QHandprint2.Execute;
  ...
  begin
    QHandprint1.Close;
    QHandprint1.SetVariable('no_nota', QMasterNO_NOTA.AsString);
    QHandprint1.SetVariable('no_reg', QMasterNO_REG.AsString);
    QHandprint1.SetVariable('kp', QDetailKD_PRODUKSI.AsString);
    QHandprint1.Execute;
