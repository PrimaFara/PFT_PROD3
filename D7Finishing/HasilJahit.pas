...
procedure THasilJahitFrm.FormClose(Sender: TObject;
...
procedure THasilJahitFrm.FormResize(Sender: TObject);
begin
...
procedure THasilJahitFrm.wwDBGrid1Enter(Sender: TObject);
begin
  If QMaster.State<> dsBrowse then QMaster.Post;
end;

procedure THasilJahitFrm.FormCreate(Sender: TObject);
begin
...
procedure THasilJahitFrm.QMasterNewRecord(DataSet: TDataSet);
begin
...
procedure THasilJahitFrm.LookRekananEnter(Sender: TObject);
begin
 DMFrm.QRekanan.Open;
end;

procedure THasilJahitFrm.LookRekananClick(Sender: TObject);
begin
   DMFrm.QRekanan.Close;
   DMFrm.QRekanan.Open;
end;

procedure THasilJahitFrm.QMasterBeforePost(DataSet: TDataSet);
begin
...
procedure THasilJahitFrm.QDetailBeforeEdit(DataSet: TDataSet);
begin
...
procedure THasilJahitFrm.LookKonstruksiEnter(Sender: TObject);
begin
  DMFrm.QKonstruksi.Open;
end;

procedure THasilJahitFrm.LookCorakEnter(Sender: TObject);
begin
  DMFrm.QCorak.Open;
end;

procedure THasilJahitFrm.LookMutuEnter(Sender: TObject);
begin
  DMFrm.QMutu.Open;
end;

procedure THasilJahitFrm.LookKonstruksiCloseUp(Sender: TObject;
...
procedure THasilJahitFrm.LookCorakCloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
  If modified then
...
procedure THasilJahitFrm.Edit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=13 then
  begin
...
procedure THasilJahitFrm.wwDBNavigator1ButtonClick(Sender: TObject);
begin
  QDetail.Close;
  LookMaster.Execute;
  QDetail.Open;
...
procedure THasilJahitFrm.QDetailNewRecord(DataSet: TDataSet);
begin
...
procedure THasilJahitFrm.BitBtn2Click(Sender: TObject);
begin
...
procedure THasilJahitFrm.ColumnHeaderBand1BeforePrint(
...
procedure THasilJahitFrm.FNo_UrutBeforeQuery(
...
procedure THasilJahitFrm.LookOperatorEnter(Sender: TObject);
begin
  DMFrm.QOperator.Open;
end;

procedure THasilJahitFrm.LookOperatorCloseUp(Sender: TObject;
  LookupTable, FillTable: TDataSet; modified: Boolean);
begin
  if modified then
...
procedure THasilJahitFrm.QMasterBeforeQuery(Sender: TOracleDataSet);
begin
  QMaster.SetVariable('kd_transaksi',QTransaksiKD_TRANSAKSI.AsString);
end;

procedure THasilJahitFrm.QTotalBeforeOpen(DataSet: TDataSet);
begin
  QTotal.SetVariable('no_reg',QMasterNO_REG.AsInteger);

end;

procedure THasilJahitFrm.wwDBGrid1UpdateFooter(Sender: TObject);
begin
  QTotal.Close;
  QTotal.Open;
