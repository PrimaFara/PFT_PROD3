...
type
  TReinspecKoreksiFrm = class(TForm)
    ...
    QLookRetur: TOracleDataSet;
    wwDBGrid3: TwwDBGrid;
    LookRetur: TwwDBLookupComboDlg;
    QLookReturNO_REG_DETAIL: TFloatField;
    QLookReturNO_ORDER: TStringField;
    QLookReturKP: TStringField;
    QLookReturKONSTRUKSI: TStringField;
    QLookReturMESIN: TStringField;
    QLookReturSISA: TFloatField;
    QLookReturKD_KONSTRUKSI: TStringField;
    wwDBEdit1: TwwDBEdit;
    Label10: TLabel;
    LookWarna: TwwDBLookupComboDlg;
    QDetailKD_WARNA: TStringField;
    QDetailWARNA: TStringField;
    Look_ket: TwwDBComboBox;
    ...
    procedure LookReturClick(Sender: TObject);
    procedure LookReturCloseUp(Sender: TObject; LookupTable,
      FillTable: TDataSet; modified: Boolean);
    ...
    procedure LookReturEnter(Sender: TObject);
...
if QMasterJNS_KOREKSI.AsString = 'RETUR KE FINISH' then
...
begin
 ...
 DMFrm.QUserTime.Open;
if wwDBComboBox3.Text='SURAT JALAN MITRA' then
...
begin
         ...
         if QMasterJNS_KOREKSI.AsString = 'SURAT JALAN MITRA' then
...
procedure TReinspecKoreksiFrm.LookReturClick(Sender: TObject);
begin
QLookRetur.Close;
QLookRetur.Open;
end;

procedure TReinspecKoreksiFrm.LookReturCloseUp(Sender: TObject;
  LookupTable, FillTable: TDataSet; modified: Boolean);
begin
if modified then
    QDetailKD_PRODUKSI.AsString:=QLookReturKP.AsString;
    QDetailKD_KONSTRUKSI.AsString:=QLookReturKD_KONSTRUKSI.AsString;
    QDetailKONSTRUKSI.AsString:=QLookReturKONSTRUKSI.AsString;
    QDetailQTY_GULUNG.AsInteger:=QLookReturNO_REG_DETAIL.AsInteger;
end;

procedure TReinspecKoreksiFrm.wwDBComboBox3CloseUp(Sender: TwwDBComboBox;
  Select: Boolean);
begin
if wwDBComboBox3.Text='SURAT JALAN MITRA' then
begin
wwDBGrid4.BringToFront;
LookMitra.Enabled:=true;
end
else
begin
wwDBGrid1.BringToFront;
LookMitra.Enabled:=false;
end;

end;

procedure TReinspecKoreksiFrm.wwDBGrid3Enter(Sender: TObject);
begin
If QMaster.State<> dsBrowse then QMaster.Post;
end;

procedure TReinspecKoreksiFrm.wwDBGrid3UpdateFooter(Sender: TObject);
begin
QTotal.Close;
  QTotal.SetVariable('no_reg',QMasterNO_REG.AsInteger);
  QTotal.Open;
...
procedure TReinspecKoreksiFrm.LookReturEnter(Sender: TObject);
begin
QLookRetur.Close;
QLookRetur.Open;
end;

procedure TReinspecKoreksiFrm.LookWarnaEnter(Sender: TObject);
begin
DMFrm.QLookWarna.Open;
end;

procedure TReinspecKoreksiFrm.LookWarnaCloseUp(Sender: TObject;
  LookupTable, FillTable: TDataSet; modified: Boolean);
begin
if modified then
...
procedure TReinspecKoreksiFrm.LookMitraEnter(Sender: TObject);
begin
QBrowseMitra.Close;
QBrowseMitra.Open;
end;
